...
# Monkey-patch: azure-monitor-opentelemetry-exporter imports LogData from
# opentelemetry.sdk._logs, but newer SDK versions removed it. Patch before import.
try:
    from opentelemetry.sdk._logs import LogData  # noqa: F401
except ImportError:
    import opentelemetry.sdk._logs as _logs_mod
    class _LogDataStub:
        """Stub for removed LogData class — only needed to satisfy import chain."""
        pass
    _logs_mod.LogData = _LogDataStub
    print("  [tracing] Patched missing LogData for azure-monitor compatibility")
...
try:
    ...
    if APP_INSIGHTS_CS:
        ...
        print("  [tracing] Azure Monitor tracing enabled (AzureMonitorTraceExporter)")
    else:
        print("  [tracing] APPLICATIONINSIGHTS_CONNECTION_STRING not set, tracing spans are local-only")
except (ImportError, Exception) as e:
    print(f"  [tracing] Azure Monitor exporter unavailable ({e}), tracing disabled")
