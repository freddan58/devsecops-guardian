...
router.post('/change-password', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    // FIXED: Using bcrypt.compare to verify password hash securely
    ...
    // FIXED: Using bcrypt.hash with salt rounds for new password
    const saltRounds = 12; // Banking-grade cost factor
    const newHash = await bcrypt.hash(new_password, saltRounds);

    db.prepare('UPDATE users SET password_hash = ? WHERE id = ?').run(newHash, req.user.id);

    // FIXED: Removed returning the password hash in response to prevent leakage
    res.json({
